...
class VGG(nnx.Module):
    ...
    def __call__(self, x: Array) -> Array:
        ...
        x = jax.image.resize(
            x, (batch_size, 7, 7, channels), method=jax.image.ResizeMethod.LINEAR
        )
...
def make_layers(
    cfg: List[Union[str, int]], batch_norm: bool = False, *, rngs: nnx.Rngs
) -> nnx.Sequential:
    ...
    for v in cfg:
        if v == "M":
            ...
            v = cast(int, v)
            conv2d = nnx.Conv(
                in_channels, v, kernel_size=(3, 3), padding="SAME", rngs=rngs
            )
            if batch_norm:
...
def _vgg(cfg: str, batch_norm: bool, *, rngs: nnx.Rngs, **kwargs):
    ...
    model = VGG(features=features, rngs=rngs, **kwargs)
    return model
